...
class AttackEffects {
    constructor(scene) {
        ...
        // Load textures
        this.textures = {
            fireball: new THREE.TextureLoader().load('assets/spear.png'),
            toast: new THREE.TextureLoader().load('assets/toast.png'),
            glass: new THREE.TextureLoader().load('assets/toast.png'),
            plate: new THREE.TextureLoader().load('assets/toast.png')
        };
    ...
    createEnemyAttackEffect(x, y, direction = -1) {
        ...
        // Create toast sprite
        const material = new THREE.SpriteMaterial({
            map: this.textures.toast,
            transparent: true,
            opacity: 1.0,
            rotation: 0
        });
        const toast = new THREE.Sprite(material);
        
        // Calculate aspect ratio
        const texture = this.textures.toast;
        const imageAspect = texture.image.width / texture.image.height;
        const baseWidth = 1.6; // Increased from 0.8 to 1.6 (2x)
        const baseHeight = baseWidth / imageAspect;
        
        // Set scale maintaining aspect ratio
        toast.scale.set(baseWidth, baseHeight, 1);
        toast.position.set(x, y, 0);
        this.scene.add(toast);
